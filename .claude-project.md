...
- `src/web-server.ts` - Query Browser web server (standalone)
- `public/` - Web server static files (HTML, CSS, JS)
...
## Query Browser Web Server

The Query Browser is a standalone web application for browsing stored queries and visualizations from the AllegroGraph query library.

### Features

- Browse all repositories on your AllegroGraph server
- View all stored queries for each repository
- Search queries by title or description
- View query details including:
  - Full SPARQL query text
  - Description and metadata
  - Creation date
- View and render visualizations:
  - Chart.js visualizations (bar, line, pie, scatter charts)
  - Interactive visualization display in modal
...
The web server uses a configuration file `web-config.json` in the project root. This allows you to easily switch between different AllegroGraph servers without setting environment variables.
...
2. Edit `web-config.json` with your AllegroGraph server details:
   ```json
   {
     "host": "flux.franz.com",
     "port": "10000",
...
- `host` - AllegroGraph hostname (e.g., `localhost`, `flux.franz.com`)
- `port` - AllegroGraph port (e.g., `10035`, `10000`)
- `username` - AllegroGraph username
- `password` - AllegroGraph password
- `catalog` - AllegroGraph catalog (use `/` for root catalog)
- `protocol` - Protocol to use (`http` or `https`)
- `webPort` - Web server port (default: `3000`)

**Note:** The `web-config.json` file is gitignored, so your credentials won't be committed.

### Architecture

The Query Browser consists of:

1. **Backend (src/web-server.ts)**: Express server with REST API endpoints
   - `/api/repositories` - List all repositories
   - `/api/repositories/:repo/queries` - Get queries for a repository
   - `/api/queries/:queryUri` - Get query details
   - `/api/queries/:queryUri/visualizations` - Get visualizations for a query
   - `/api/visualizations/:vizUri/render` - Render visualization as HTML

2. **Frontend (public/)**: Single-page application
   - `index.html` - Main HTML structure
   - `app.js` - Client-side JavaScript (fetch API, DOM manipulation)
   - `style.css` - Responsive CSS styling

3. **Data Source**: AllegroGraph query-library repository
   - Queries SPARQL endpoints to retrieve stored queries
   - Uses query library vocabulary (`query:`, `viz:`, `dc:` prefixes)

### Use Cases

- Browse the complete catalog of proven SPARQL queries
- Share visualizations with team members via browser
- Review and reuse successful query patterns
- Quality assurance for stored queries before using in production
- Create a knowledge base of domain-specific queries
